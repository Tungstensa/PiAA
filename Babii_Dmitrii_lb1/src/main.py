...
def run_benchmarks():
    ...
    c_coeffs = np.polyfit(comp_ns, comp_times, 2)
    x_comp_smooth = np.linspace(min(comp_ns), max(comp_ns), 200)
    theor_comp = np.polyval(c_coeffs, x_comp_smooth)
    
    log_p_times = np.log(prime_times)
    alpha, beta = np.polyfit(prime_ns, log_p_times, 1)
    x_prime_smooth = np.linspace(min(prime_ns), max(prime_ns), 200)
    theor_prime = np.exp(alpha * x_prime_smooth + beta)

    plt.figure(figsize=(14, 10))

    ax1 = plt.subplot(2, 1, 1)
    ax1.plot(even_ns, even_times, 'g-o', label="Четное-практика")
    ax1.plot(even_ns, theor_even, 'g--', label="Четное-теория (O(1))", linewidth=2)

    ax1.plot(comp_ns, comp_times, 'b-o', label="Составное-практика")
    ax1.plot(x_comp_smooth, theor_comp, 'b--', label="Составное-теория (полиномиальное)", linewidth=2)
    ...
    ax2.plot(x_prime_smooth, theor_prime, 'r--', label="Простое-теория (O(e^N))", linewidth=2)
